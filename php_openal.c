...
//Stuff for custom loading waves
/*
...
typedef struct  {
  char chunkID[4];
  ALsizei chunkSize;//size not including chunkSize or chunkID
  char format[4];
} RIFF_Header;
...
typedef struct  {
  ...
  ALsizei subChunk2Size; //Stores the size of the data block
} WAVE_Data;
...
PHP_FUNCTION(openal_buffer_loadwav)
{
    ...
    if ((riff_header.chunkID[0] != 'R' ||
        ...
        (riff_header.format[0] != 'W' ||
		 ...
		 {
             php_error_docref(NULL TSRMLS_CC, E_WARNING, "The file (%s) is not in wav format.", wavfile);
			 if (soundFile != NULL)
				fclose(soundFile);
			 RETURN_FALSE;
		}
    ...
    if (wave_data.subChunkID[0] != 'd' ||
        ...
        wave_data.subChunkID[3] != 'a')
             {
             php_error_docref(NULL TSRMLS_CC, E_WARNING, "The file (%s) has invalid data headers.", wavfile);
			 if (soundFile != NULL)
				fclose(soundFile);
			 RETURN_FALSE;
		}
    ...
    data = (unsigned char*) emalloc(wave_data.subChunk2Size);
 
    // Read in the sound data into the soundData variable
    if (!fread(data, wave_data.subChunk2Size, 1, soundFile))
	{
             php_error_docref(NULL TSRMLS_CC, E_WARNING, "There was an error reading the audio data in (%s).", wavfile);
			 if (soundFile != NULL)
				fclose(soundFile);
			 RETURN_FALSE;
	}
...
static const zend_function_entry openal_functions[] = {
...
PHP_MINFO_FUNCTION(openal)
{
	...
	php_info_print_table_row(2, "Can load WAV?", "Yes");
